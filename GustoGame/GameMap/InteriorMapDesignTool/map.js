$(document).ready(function(){
	...
	var selectedColor = "#cfa34c";
	var selectedGroundObjectColor = "black"
	var selectedTerrainKey = "null";
	...
	$('.terrain').click(function() {
		...
		if (selectedTerrainKey.substr(-1) === "w") {
			wallPiece = true;
		}
		else {
			wallPiece = false;
	...
	$('#map').on('mousemove', '.tile', function(e){
		...
		if (e.buttons === 1) {
	    	...
	    	$(this).text(selectedSittingObjectKey);
	...
	function LoadMap(mapdata, width, height, multX, multY, tileSize) {
		...
		for (var i = 0; i < rows; i++) {
			for (var j = 0; j < cols; j++) {
				...
				var entry = tileDict['data'][index];
				
				if (!entry.hasOwnProperty('sittingObject')) {
					var entryTer = tileDict['data'][index]['terrainPiece'];
					entry= {};
	...
	function BuildMap(width, height, multX, multY, tileSize) {
		...
		tileDict['multX'] = multX;
		tileDict['multY'] = multY;
		tileDict['data'] = {};
		$('#map').empty();
